...
from Poisson_Image_Editing import utils

class PoissonSeamlessCloner:
    #@profile
    def __init__(self, mask,source,target, solver, scale):
        self.mask = mask
        self.src_rgb = source
        self.target_rgb = target
...
if __name__ == "__main__":
    ...
    cloner = PoissonSeamlessCloner(mask,source,target,"spslove", 1.0)

    if args.grayscale:
        img = cloner.poisson_blend_gray("alpha", 1.0)
    else:
